...
const mock_flight_catalog_1 = require("./mock-flight-catalog");
...
function routeMultiplier(dest) {
    return mock_flight_catalog_1.MOCK_ROUTE_MULTIPLIER[dest] ?? 1.2;
}
function filterTemplatesForCabin(templates, cabin) {
...
function buildReturnSegments(outbound, returnDate, airlineCode, seed) {
    const airline = mock_flight_catalog_1.MOCK_AIRLINES[airlineCode] || {
        code: airlineCode,
        name: airlineCode,
        nameAr: airlineCode,
    };
    ...
    const segments = segs.map((s, index) => {
        ...
        if (index === 0)
            firstDepartOffset = departOffset;
        lastArriveOffset = departOffset + duration;
        const dep = (0, mock_flight_catalog_1.addMinutesToIsoDate)(returnDate, departOffset);
        const arr = (0, mock_flight_catalog_1.addMinutesToIsoDate)(returnDate, departOffset + duration);
        return {
            from: s.to,
            to: s.from,
            date: returnDate,
...
class MockFlightProvider {
    ...
    async searchFlights(params) {
        ...
        const seed = hashSeed(`${origin}-${destination}-${params.departDate}-${params.adults}-${params.cabinClass || "economy"}`);
        ...
        const templates = filterTemplatesForCabin(mock_flight_catalog_1.MOCK_FLIGHT_TEMPLATES, params.cabinClass);
        const offers = templates.map((tpl, index) => {
            const airline = mock_flight_catalog_1.MOCK_AIRLINES[tpl.airlineCode] || {
            ...
            const segments = tpl.segments.map((segTpl, segIndex) => {
                const from = segTpl.from === "KWI"
                    ? origin
                    : segTpl.from === "DEST"
                        ? destination
                        : segTpl.from;
                const to = segTpl.to === "DEST"
                    ? destination
                    : segTpl.to === "KWI"
                        ? origin
                        : segTpl.to;
                const duration = segTpl.durationMin + ((seed + index + segIndex) % 25) - 10;
                const safeDuration = Math.max(60, duration);
                const dep = (0, mock_flight_catalog_1.addMinutesToIsoDate)(params.departDate, segTpl.departOffsetMin + ((seed + index) % 20));
                const arr = (0, mock_flight_catalog_1.addMinutesToIsoDate)(params.departDate, segTpl.departOffsetMin + ((seed + index) % 20) + safeDuration);
            ...
            const costAmountMinor = (0, scenario_1.toMinor)(totalMajor, currency);
            const scenario = tpl.scenario || "normal";
            const scenarioTag = scenario === "price_change"
                ? "PRICE-CHANGE"
                : scenario === "sold_out"
                    ? "SOLD-OUT"
            ...
            const returnBuilt = isRoundTrip && params.returnDate
                ? buildReturnSegments(segments, params.returnDate, tpl.airlineCode, seed + index)
                : null;
            const returnSegments = returnBuilt?.segments || [];
            const returnDurationMinutes = returnBuilt?.durationMinutes || 0;
            const destLabel = mock_flight_catalog_1.MOCK_DESTINATION_LABELS[destination] || destination;
            const tripLabel = isRoundTrip ? "ذهاب وعودة" : "ذهاب فقط";
            return {
                providerKey: this.providerKey,
                providerOfferRef,
                description: `${airline.nameAr} ${origin} → ${destination} · ${tripLabel} · ${params.departDate}${params.returnDate ? ` → ${params.returnDate}` : ""} · ${adults} بالغ`,
                costAmountMinor,
                currency,
                ...
                raw: {
                    ...
                    airlineCode: airline.code,
                    cabin: tpl.cabin,
                    duration: (0, mock_flight_catalog_1.minutesToDuration)(Math.max(totalDuration, 60)),
                    ...
                    returnDuration: returnDurationMinutes > 0
                        ? (0, mock_flight_catalog_1.minutesToDuration)(returnDurationMinutes)
                        : null,
                    ...
                    fare: {
                        baseAmountMinor: (0, scenario_1.toMinor)(fareMajor * adults, currency),
                        taxesAmountMinor: (0, scenario_1.toMinor)(taxesMajor * adults, currency),
                        currency,
                    },
                    baggage: tpl.baggage,
    ...
    async revalidateOffer(offer) {
        const scenario = (0, scenario_1.scenarioFromOfferRef)(offer.providerOfferRef) ||
        ...
        if (scenario === "price_change") {
            const bumpMajor = 6.5;
            const bump = (0, scenario_1.toMinorKwd)(bumpMajor);
            // Scale bump if currency isn't KWD-like
            const scaled = (0, scenario_1.currencyExponent)(offer.currency) === 3
                ? bump
    ...
    async createBooking(offer, _passengers) {
        const scenario = (0, scenario_1.scenarioFromOfferRef)(offer.providerOfferRef) ||
            String(offer.raw?.scenario || "normal");
        if (scenario === "sold_out" || scenario === "unavailable") {
            return {
        ...
        if (scenario === "provider_fail") {
            throw new Error("فشل مزود الخدمة التجريبي (MOCK-PROVIDER-FAIL) — أعد المحاولة أو اختر عرضًا آخر");
        }
        return {
...
//# sourceMappingURL=mock-flight-provider.js.map