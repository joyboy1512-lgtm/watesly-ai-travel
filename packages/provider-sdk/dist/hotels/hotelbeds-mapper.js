...
exports.extractHotelbedsRateOptions = extractHotelbedsRateOptions;
exports.mapHotelbedsToOffer = mapHotelbedsToOffer;
exports.mapCheckratesToOffer = mapCheckratesToOffer;
exports.findMappedRate = findMappedRate;
...
function rateMajor(rate, hotel) {
    const raw = rate.net ?? rate.sellingRate ?? rate.hotelSellingRate ?? hotel.minRate;
    const n = Number(raw);
    return Number.isFinite(n) && n > 0 ? n : 0;
}
function hasFreeCancellation(policies) {
    if (!policies?.length)
        return false;
    return policies.some((p) => Number(p.amount || 0) === 0);
}
function mapTaxes(taxes, providerCurrency = "EUR", displayCurrency = "KWD") {
    ...
    return {
        ...
        items: taxes.taxes.map((t) => ({
            type: t.type,
            amount: (0, hotelbeds_currency_1.convertHotelbedsAmount)(Number(t.amount ?? t.clientAmount ?? 0), String(t.currency ?? t.clientCurrency ?? providerCurrency), displayCurrency),
...
function mapDailyRates(rate, checkIn, convert) {
    if (!rate.dailyRates?.length)
        return undefined;
...
function mapPromotions(rate) {
    ...
    const fromOffers = (rate.offers || []).map((o) => ({
        code: o.code,
        name: o.name,
        remark: o.amount != null && Number(o.amount) !== 0 ? `خصم ${o.amount}` : o.name,
    }));
...
function mapRate(rate, room, hotel, providerCurrency, displayCurrency, checkIn) {
    if (!rate.rateKey)
        return null;
    const netRaw = rateMajor(rate, hotel);
    if (netRaw <= 0)
        return null;
    const convert = (value, from = providerCurrency) => (0, hotelbeds_currency_1.convertHotelbedsAmount)(value, from, displayCurrency);
    const boardCode = String(rate.boardCode || "RO");
    const boardName = (0, shared_1.boardLabelAr)(boardCode, rate.boardName);
    ...
    return {
        ...
        net: convert(netRaw),
        netBasis: "stay",
        sellingRate: Number.isFinite(selling) ? convert(selling) : undefined,
        currency: displayCurrency,
...
function extractHotelbedsRateOptions(hotel, providerCurrency, displayCurrency, checkIn) {
    const rooms = [];
    const rateOptions = [];
    for (const room of hotel.rooms || []) {
        const mappedRates = [];
        for (const rate of room.rates || []) {
            const mapped = mapRate(rate, room, hotel, providerCurrency, displayCurrency, checkIn);
            if (mapped) {
    ...
    rooms.sort((a, b) => (a.rates[0]?.net ?? Infinity) - (b.rates[0]?.net ?? Infinity));
    return { rooms, rateOptions };
}
function sourceMeta(liveMode, baseUrl) {
    const sandbox = /test\.hotelbeds\.com/i.test(baseUrl || process.env.HOTELBEDS_BASE_URL || "");
    if (!liveMode) {
        return { source: "mock", sourceLabel: "تجريبي" };
    }
    return sandbox
        ? { source: "hotelbeds-sandbox", sourceLabel: "نتيجة تجريبية من Hotelbeds Sandbox" }
        : { source: "hotelbeds-live", sourceLabel: "Hotelbeds Live" };
}
function mapHotelbedsToOffer(input) {
    ...
    const requested = (0, hotelbeds_currency_1.hotelbedsDisplayCurrency)(params.currency);
    const displayCurrency = (0, hotelbeds_currency_1.canConvertHotelbedsCurrency)(providerCurrency, requested)
        ? requested
        : providerCurrency;
    const checkIn = hotel.checkIn || params.checkInDate;
    const checkOut = hotel.checkOut || params.checkOutDate;
    const { rooms, rateOptions } = extractHotelbedsRateOptions(hotel, providerCurrency, displayCurrency, checkIn);
    ...
    const promotions = [
        ...new Set(rateOptions.flatMap((r) => r.promotions.map((p) => p.name || p.remark || "").filter(Boolean))),
    ];
    ...
    const enriched = (0, hotelbeds_content_mapper_1.enrichDetailsFromContent)({ details, content, searchCenter, facilityCatalog });
    ...
    return {
        ...
        description,
        costAmountMinor: (0, types_1.amountToMinor)(cheapest.net, displayCurrency),
        currency: displayCurrency,
...
function mergeRooms(previous, next) {
    const prevRooms = Array.isArray(previous) ? previous : [];
    const nextRooms = Array.isArray(next) ? next : [];
...
function mapCheckratesToOffer(offer, hotel, selectedRateKey) {
    ...
    const rates = (Array.isArray(nextRaw.rateOptions)
        ? nextRaw.rateOptions
        : []);
    const selected = rates.find((r) => r.rateKey === selectedRateKey) || rates[0];
    ...
    return {
        ...
        costAmountMinor: selected
            ? (0, types_1.amountToMinor)(selected.net, offer.currency)
...
function findMappedRate(offer, rateKey) {
...
//# sourceMappingURL=hotelbeds-mapper.js.map