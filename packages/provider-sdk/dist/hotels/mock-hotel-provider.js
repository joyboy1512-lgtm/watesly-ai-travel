...
const mock_hotel_catalog_1 = require("./mock-hotel-catalog");
const mock_hotel_rates_1 = require("./mock-hotel-rates");
...
class MockHotelProvider {
    ...
    async searchHotels(params) {
        ...
        const seed = hashSeed(`${params.location}-${params.checkInDate}-${params.checkOutDate}-${params.adults}`);
        const currency = (params.currency || "KWD").toUpperCase();
        const expiresAt = new Date(Date.now() + 45 * 60 * 1000).toISOString();
        const nights = Math.max(1, Math.round((new Date(params.checkOutDate).getTime() -
            new Date(params.checkInDate).getTime()) /
            (24 * 60 * 60 * 1000)));
        const locUpper = params.location.trim().toUpperCase();
        const destCode = mock_flight_catalog_1.MOCK_DESTINATION_LABELS[locUpper]
            ? locUpper
            : Object.entries(mock_flight_catalog_1.MOCK_DESTINATION_LABELS).find(([, label]) => params.location.includes(label))?.[0] || locUpper.slice(0, 3);
        const label = geo?.label || mock_flight_catalog_1.MOCK_DESTINATION_LABELS[destCode] || params.location;
        const hotels = (0, mock_hotel_catalog_1.hotelsForDestination)(destCode, label);
        // Attach package/tour/transfer snapshots for quote enrichment (not sold as separate hotel rows)
        const ancillaries = (0, shared_1.buildMockAncillaryOffers)({
        ...
        return hotels.map((hotel, index) => {
            ...
            const nightMajor = currency === "KWD" || currency === "BHD" || currency === "OMR"
                ? hotel.nightRateKwd * variance
                : hotel.nightRateKwd * variance * 10;
            const cost = (0, scenario_1.toMinor)(nightMajor * nights, currency);
            ...
            const { rooms, rateOptions } = (0, mock_hotel_rates_1.buildMockHotelRateTree)({
                hotel,
                providerOfferRef,
                nightMajor,
                currency,
            ...
            const dist = geo && hotelLat != null && hotelLng != null
                ? (0, hotelbeds_geo_1.buildDistanceInfo)({
                    hotelLat,
                    hotelLng,
                    center: geo,
                    label: label,
            ...
            return {
                ...
                raw: {
                    ...
                    latitude: hotelLat,
                    longitude: hotelLng,
                    mapUrl: hotelLat != null && hotelLng != null
                    ...
                    facilityLabels: hotel.facilities.map((f) => f === "wifi" ? "واي‑فاي" : f === "parking" ? "موقف" : f === "pool" ? "مسبح" : f === "spa" ? "سبا" : f === "gym" ? "جيم" : f),
                    nights,
                    minRate,
                    maxRate,
                    ...
                    promotions: rateOptions.flatMap((r) => r.promotions.map((p) => p.name || p.remark || "").filter(Boolean)),
                    location: label,
                    ...
                    relatedAncillaries: ancillaries.filter((a) => ["transfer", "tour", "package"].includes(a.serviceType)),
                },
            };
        });
    }
...
//# sourceMappingURL=mock-hotel-provider.js.map