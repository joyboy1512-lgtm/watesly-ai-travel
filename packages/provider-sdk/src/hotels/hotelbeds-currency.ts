...
const SOURCE_MARKET: Record<string, string> = {
  KWD: "KW",
  SAR: "SA",
  AED: "AE",
  BHD: "BH",
...
const DEFAULT_TO_KWD: Record<string, number> = {
  KWD: 1,
  EUR: 0.355,
  USD: 0.307,
  GBP: 0.412,
...
function envRate(code: string, fallback: number): number {
  const raw = Number(process.env[`FX_${code}_KWD`]);
  return Number.isFinite(raw) && raw > 0 ? raw : fallback;
}

function unitsPerKwd(code: string): number | null {
...
export function hotelbedsDisplayCurrency(requested?: string | null): string {
  return (
    requested ||
...
export function hotelbedsSourceMarket(currency?: string | null): string {
...
export function convertHotelbedsAmount(
  amount: number,
  fromCurrency: string,
  toCurrency: string,
): number {
  ...
  if (fromKwd == null || toKwd == null || toKwd <= 0) return amount;
...
export function canConvertHotelbedsCurrency(from: string, to: string): boolean {
