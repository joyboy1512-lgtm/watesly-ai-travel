export type GeoCenter = { latitude: number; longitude: number; label?: string };
...
const CITY_POIS: Record<string, PointOfInterest[]> = {
  DXB: [
...
export function resolvePois(destinationCode?: string, label?: string): PointOfInterest[] {
  const code = (destinationCode || "").toUpperCase();
  if (CITY_POIS[code]) return CITY_POIS[code];
  const upper = (label || "").toUpperCase();
  for (const [key, pois] of Object.entries(CITY_POIS)) {
    if (upper.includes(key)) return pois;
  }
  if (label?.includes("دبي")) return CITY_POIS.DXB || [];
  if (label?.includes("الكويت")) return CITY_POIS.KWI || [];
  return [];
}

export function buildDistanceInfo(input: {
  hotelLat?: number;
  hotelLng?: number;
  center: GeoCenter;
  ...
  if (hotelLat == null || hotelLng == null) {
    return { distanceToCenterKm: undefined, distanceToCenterLabel: undefined, poiDistances: [] as Array<{ nameAr: string; km: number; label: string }> };
  }
