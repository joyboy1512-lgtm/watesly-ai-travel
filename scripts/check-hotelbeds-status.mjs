...
// Activities often has no /status — prove keys with a 1-item search probe.
{
  const apiKey = process.env.HOTELBEDS_ACTIVITY_API_KEY;
  const apiSecret = process.env.HOTELBEDS_ACTIVITY_API_SECRET;
  if (apiKey && apiSecret) {
    const from = new Date(Date.now() + 21 * 864e5).toISOString().slice(0, 10);
    const to = new Date(Date.now() + 28 * 864e5).toISOString().slice(0, 10);
    const url = `${activityBase.replace(/\/$/, "")}/activity-api/3.0/activities`;
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          filters: [{ searchFilterItems: [{ type: "destination", value: "DXB" }] }],
          from,
          to,
          language: "en",
          paxes: [{ age: 30 }],
      ...
      const n = Array.isArray(json.activities) ? json.activities.length : 0;
      console.log(
        `[activity] search probe DXB → HTTP ${res.status} items=${n}`,
      );
    } catch (err) {
      console.log(
        `[activity] search probe → ERROR ${err instanceof Error ? err.message : err}`,
      );
    }
  }
}
